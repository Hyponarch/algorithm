...
final class DualPivotQuicksort {
     ...
     *
     * 计算并行归并的double（？）深度
     * 如果任务在排序前被划分，则depth是负数
     *
     * @param parallelism the parallelism level 并行化等级
     * @param size the target size  数组大小/4096，每个并行线程至少4096个数？
     * @return the depth of parallel merging    并行归并的深度
     */
    private static int getDepth(int parallelism, int size) {
        //初始为0
        int depth = 0;
        //parallelism /= 8>0 && size /= 4 >0，depth = -log(parallelism)/log(3) = -log(size)/log(2)
    ...
    static void sort(int[] a, int parallelism, int low, int high) {
        int size = high - low;
        //当并行化等级>1而且数字size大于4096时，才考虑并行化
        if (parallelism > 1 && size > MIN_PARALLEL_SORT_SIZE) {
            //并行化的深度
            int depth = getDepth(parallelism, size >> 12);
            //depth不等于0时，为一个原数组大小的数组
     ...
     *
     * @param sorter parallel context   并行上下文
     * @param a the array to be sorted
     * @param bits the combination of recursion depth and bit flag, where
     *        the right bit "0" indicates that array is the leftmost part
     *             递归深度和bit flag的组合，最右位为0表示当前数组是最左部分
     * @param low the index of the first element, inclusive, to be sorted
     * @param high the index of the last element, exclusive, to be sorted
     */
    static void sort(Sorter sorter, int[] a, int bits, int low, int high) {
        while (true) {
             ...
             * 在小尺寸的非最左部分，进行混合插入排序
             */
            //数组大小小于65+bits时，且 不是最左
            if (size < MAX_MIXED_INSERTION_SORT_SIZE + bits && (bits & 1) > 0) {
                //high - 3 * ((size >> 5) << 3) size/32*8，相当于/4，且二进制第4、5为归零
     ...
     *
     * 混合插入排序由简单插入排序、pin插入排序、pair插入排序组合而成
     *
     ...
     *
     * 在双基准快排中，左侧的pivot充当了哨兵的作用，因为它比给定范围内的任何元素都要小
     * 得益于此，每轮迭代中代价高昂的左侧边界检查可以被省略，除了最左部分的调用
     *
     * @param a the array to be sorted
     * @param low the index of the first element, inclusive, to be sorted
     ...
     */
    private static boolean tryMergeRuns(DualPivotQuicksort.Sorter sorter, int[] a, int low, int size) {
    ...
    private static int[] mergeRuns(int[] a, int[] b, int offset,
        ...
        if (parallel && hi - lo > MIN_RUN_COUNT) {
            DualPivotQuicksort.RunMerger merger = new DualPivotQuicksort.RunMerger(a, b, offset, 0, run, mi, hi).forkMe();
        ...
        if (parallel) {
            new DualPivotQuicksort.Merger(null, dst, k, a1, lo1, hi1, a2, lo2, hi2).invoke();
        } else {
     ...
     */
    private static void mergeParts(DualPivotQuicksort.Merger merger, int[] dst, int k,
...
// [long]
    ...
    static void sort(long[] a, int parallelism, int low, int high) {
        ...
        if (parallelism > 1 && size > MIN_PARALLEL_SORT_SIZE) {
            ...
            new DualPivotQuicksort.Sorter(null, a, b, low, size, low, depth).invoke();
        } else {
            sort(null, a, 0, low, high);
        }
    }
     ...
     * @param high the index of the last element, exclusive, to be sorted
     */
    static void sort(DualPivotQuicksort.Sorter sorter, long[] a, int bits, int low, int high) {
        while (true) {
            int end = high - 1, size = high - low;
     ...
     * @return true if finally sorted, false otherwise
     */
    private static boolean tryMergeRuns(DualPivotQuicksort.Sorter sorter, long[] a, int low, int size) {
     ...
     * @param hi2 the end index of the second part, exclusive
     */
    private static void mergeParts(DualPivotQuicksort.Merger merger, long[] dst, int k,
...
// [float]
     ...
     * @param high the index of the last element, exclusive, to be sorted
     */
    static void sort(DualPivotQuicksort.Sorter sorter, float[] a, int bits, int low, int high) {
        while (true) {
            int end = high - 1, size = high - low;
     ...
     * @return true if finally sorted, false otherwise
     */
    private static boolean tryMergeRuns(DualPivotQuicksort.Sorter sorter, float[] a, int low, int size) {
     ...
     * @param hi2 the end index of the second part, exclusive
     */
    private static void mergeParts(DualPivotQuicksort.Merger merger, float[] dst, int k,
...
// [double]
     ...
     * @param high the index of the last element, exclusive, to be sorted
     */
    static void sort(DualPivotQuicksort.Sorter sorter, double[] a, int bits, int low, int high) {
        while (true) {
            int end = high - 1, size = high - low;
     ...
     * @return true if finally sorted, false otherwise
     */
    private static boolean tryMergeRuns(DualPivotQuicksort.Sorter sorter, double[] a, int low, int size) {
     ...
     * @param hi2 the end index of the second part, exclusive
     */
    private static void mergeParts(DualPivotQuicksort.Merger merger, double[] dst, int k,
...
// [class]
    ...
    private static final class Sorter extends CountedCompleter<Void> {
        ...
        public final void compute() {
            if (depth < 0) {
                setPendingCount(2);
                int half = size >> 1;
                new DualPivotQuicksort.Sorter(this, b, a, low, half, offset, depth + 1).fork();
                new DualPivotQuicksort.Sorter(this, b, a, low + half, size - half, offset, depth + 1).compute();
            } else {
                if (a instanceof int[]) {
        ...
        public final void onCompletion(CountedCompleter<?> caller) {
            if (depth < 0) {
                ...
                new DualPivotQuicksort.Merger(null,
                        a,
                        src ? low : low - offset,
                        b,
                        src ? low - offset : low,
        ...
        private void forkSorter(int depth, int low, int high) {
            ...
            new DualPivotQuicksort.Sorter(this, a, b, low, high - low, offset, depth).fork();
        }
    }
    ...
    private static final class RunMerger extends RecursiveTask<Object> {
        ...
        private DualPivotQuicksort.RunMerger forkMe() {
            fork();
            return this;
        }
